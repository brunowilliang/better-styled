...
<Note>
  Context values are only provided when a parent component with `context` receives variant props. If no variant props are passed, children read from their own `defaultVariants`.
</Note>
